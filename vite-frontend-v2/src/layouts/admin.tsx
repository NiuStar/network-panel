...
export default function AdminLayout({
  ...
  const showUpgrade =
    !!serverVersion &&
    !!latestTag &&
    normalizeVer(serverVersion) !== normalizeVer(latestTag);
  ...
  const doUpgrade = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/v1/version/upgrade-stream", {
        method: "GET",
        headers,
        credentials: "include",
      });
      ...
      setUpgradeDoneFlag(upgradeCompleted);
    }
  };
  ...
  const filteredMenuItems = menuItems.filter((item) => {
    ...
    if (roleId === 2 && (item.path === "/node" || item.path === "/user"))
      return false;
  ...
  return (
    <div
      className={`flex ${isMobile ? "min-h-screen" : "h-screen"} apple-shell`}
    >
      ...
      >
        ...
        <div className="apple-divider mx-3" />

        {/* 菜单导航 */}
        <nav className="flex-1 px-3 py-5 overflow-y-auto">
          <div className="text-[11px] uppercase tracking-[0.2em] text-default-400 px-2 mb-2">
            Menu
          </div>
                ...
                <li key={item.path}>
                  <button
                    className={`
                       ...
                       transition-colors duration-200 min-h-[44px] apple-menu-item
                       ${isActive ? "apple-menu-item-active" : ""}
                     `}
      ...
      >
        {/* 顶部导航栏 */}
        <header className="apple-topbar h-14 flex items-center justify-between px-4 lg:px-6 relative z-10 mx-4 mt-4 rounded-2xl">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm font-semibold">Network Panel</span>
              {siteConfig.build_date && (
                <span className="np-pill">
                  v{siteConfig.version || siteConfig.app_version} · build{" "}
                  {new Date(siteConfig.build_date).toLocaleDateString()}
                </span>
              )}
              {serverVersion && (
                <span className="np-pill np-pill-strong">
          ...
          <div className="flex items-center gap-3">
            <Button
              ...
              onPress={() => {
                ...
                window.open(`${base.replace(/\/+$/, "")}/swagger`, "_blank");
              }}
            >
              接口文档
            </Button>
            ...
            <Button
              size="sm"
              variant="flat"
              onPress={() => setUpgradeLogOpen(true)}
            >
              升级日志
            </Button>
            {/* 用户菜单 */}
            <Dropdown disableAnimation placement="bottom-end">
              <DropdownTrigger>
                <Button
        ...
        {/* 主内容 */}
        <main
          className={`flex-1 apple-content ${isMobile ? "" : "overflow-y-auto"}`}
        >
          {children}
        </main>
      </div>
