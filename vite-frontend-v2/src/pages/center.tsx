...
const Section = ({
  ...
  const items = useMemo(() => {
    ...
    if (Array.isArray(data.items)) {
      ...
      list = Object.values(data.items as Record<string, HBItem>).filter(
        (it) => it && typeof it === "object",
      );
    }
    return [...list].sort((a, b) => b.firstSeenMs - a.firstSeenMs);
  }, [data]);
  const itemsShapeInvalid = useMemo(() => {
    if (!data) return false;
    const raw: any = (data as any).items;
  ...
  return (
    <Card className="np-card w-full">
      ...
      <CardBody className="overflow-x-auto">
        {itemsShapeInvalid && (
          <div className="mb-3 text-xs text-warning-600">
            ⚠️ 返回数据结构异常：items 不是数组/对象，已自动忽略。
          </div>
        )}
        {!data ? (
  ...
  return (
    <div className="np-page">
      <div className="np-page-header">
        <div>
          <h2 className="np-page-title">{header}</h2>
          <p className="np-page-desc">
            用于统计 agent / 中控程序的版本、系统、架构与存活状态
          </p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex rounded-full overflow-hidden np-soft">
            <button
