...
import { needsPanelAddress } from "@/utils/panel";
...
const ForwardNewPage = lazy(() => import("@/pages/forward-new"));
...
const PanelAddressGuard = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener("panel-address-required", handler);
    return () => window.removeEventListener("panel-address-required", handler);
  }, [navigate]);

  useEffect(() => {
    if (needsPanelAddress() && window.location.pathname !== "/settings") {
      navigate("/settings");
    }
  }, [navigate]);
