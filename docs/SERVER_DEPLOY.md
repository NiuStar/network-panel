...
- 前端静态资源：安装脚本会自动从 GitHub Release 下载 `frontend-dist.zip` 并解压至 `/opt/network-panel/public/`；离线环境可在本地 `vite-frontend-v2` 目录执行 `npm install && npm run build`，将 `dist/*` 手动复制到该目录
