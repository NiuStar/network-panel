...
func BuildPathServicesForReconnect(rootNode int64) map[int64][]map[string]any {
	...
	getUsed := func(nodeID int64) map[int]bool {
		if m, ok := usedPortsCache[nodeID]; ok {
			return m
		}
	...
	for _, r := range rows {
		...
		if !involves && r.TType == 2 && r.OutNodeID != nil && *r.OutNodeID == rootNode {
			involves = true
		}
		if !involves {
			continue
		...
		if r.TType == 1 {
			...
			for i := 0; i < len(hops); i++ {
				...
				svcName := name
				if i > 0 {
					svcName = fmt.Sprintf("%s_mid_%d", name, i-1)
				}
